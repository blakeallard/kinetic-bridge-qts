...
4. **No search criteria at all on `Quote_Request`** — not a function call, and not even a plain local variable. Both `Quote_Request[ID == p_quote_id.toLong()]` and `Quote_Request[ID == quote_id_long]` (a pre-computed local variable) failed with "Improper Statement" in this Creator instance (see Incident 3 below), even though the plain-variable form works fine for every other module (`Item_Master[Part_Number == part]`, `FX_Rates_Cache[Currency == currency]`, etc. are all still fine — this restriction is specific to `Quote_Request[ID == ...]`, not a general Deluge rule). For `Quote_Request` specifically: loop over every record unfiltered (`for each quote in Quote_Request`) and compare `quote.ID` against the target value inside an `if` in the loop body instead. For every other module, a plain local variable inside `[...]` criteria remains fine — only avoid function calls inside those brackets (e.g. never write `Module[Field == some_call()]`).
...
**Open question, not yet resolved**: `functions/fn_sync_to_sheet.deluge`, `fn_generate_pdf.deluge`, `fn_sync_to_crm.deluge`, and `fn_send_to_sign.deluge` all still use the filtered `Quote_Request[ID == ...]` form (a pre-computed variable in `fn_sync_to_sheet.deluge`'s case, a direct function call in the other three). Whether this form is actually safe for those functions, or only appears safe because they have not been redeployed/retested since this parser issue was discovered, is unconfirmed. Recommend testing the filtered form against a save in Creator before or during each of those functions' next deployment, and applying the same unfiltered-loop-plus-if treatment if any of them also fail.
...
`fn_get_discount.deluge` contains a ternary (`p_is_software ? "Software" : "Hardware"`) and some non-ASCII characters, but per `QTS_PROJECT_STATUS.md` this function predates the current session's changes and is presumably already deployed and working — it has not been touched or redeployed in this session, so its existing ternary has not (yet) caused a failure. If `fn_get_discount.deluge` is ever edited and redeployed, apply the same house rules above first rather than assuming its current form is safe simply because it hasn't failed yet.

`fn_generate_pdf.deluge` and `fn_sync_to_crm.deluge` also still contain a handful of em-dashes; same reasoning — not touched this session, not redeployed, left as-is.
...
- `fn_sync_to_sheet.deluge`: two chained `ifnull(...) == "value"` expressions (house rule 6) decomposed into variable-then-compare form. No ternary or compound boolean was present.
