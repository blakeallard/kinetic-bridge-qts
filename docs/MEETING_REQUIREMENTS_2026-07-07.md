...
Input: meeting summary provided by Blake, 2026-07-07. Reconciled against `README.md`, `QTS_PROJECT_STATUS.md`, `import_preview/`, `audit/reports/`, and public Lithium Balance sources (researched 2026-07-07).

**Source priority** (per meeting instruction):

1. Transcript = business intent
2. Lithium Balance datasheets/product pages = engineering rules
...
- **i-BMS15/6 — TRANSCRIPT CORRECTED.** The meeting note said "15 series channels, up to 6 boards daisy-chained." The vendor page says the i-BMS15 has **8–15 voltage channels** (17–60 V) and the "/6" means **up to 6 battery packs in parallel with Hot Swap** — parallel packs, not series daisy-chaining ([i-BMS15 product page](https://lithiumbalance.com/products/i-bms15/)). Quantity driver is parallel pack count, not series cell count.
- **c-BMS24 / c-BMS24X:** up to 24 cells each, 0–120 VDC measurement; c-BMS24X supports up to 10 parallel packs ([c-BMS24](https://lithiumbalance.com/products/c-bms24/), [c-BMS24X](https://lithiumbalance.com/products/c-bms24x/)). Single-board products — no series-cell CMU math.
...
**Research limitations:** helpdesk.lithiumbalance.com and sensata.com return HTTP 403 to automated fetches; distributor-hosted PDF datasheets were not machine-readable with available tooling. Engineering numbers above come from lithiumbalance.com product-page HTML. Re-verify against the official PDFs before the kit BOM pass ([datasheet index](https://helpdesk.lithiumbalance.com/hc/en-us/articles/8616017930258-List-of-all-off-the-shelf-Sensata-Technologies-BMS-and-their-datasheets), browse manually).
...
**Meeting:** Import/update logic should eventually handle new quarterly Lithium Balance sheets: changed prices, changed quantity tiers, distributor restrictions, currencies, new items, discontinued items, delivery-stop/changed flags. Before building a custom full upload utility, research whether Zoho has native Price Books / price-list / matrix-pricing upload functionality, and document why any custom build doesn't duplicate it.
