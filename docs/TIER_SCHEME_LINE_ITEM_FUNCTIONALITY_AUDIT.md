...
- `docs/AcmeBMS_NBMS_CBMS24_PRICE_QC.md`, `docs/AcmeBMS_REFERENCE_PRICE_QC.md` — prior QC findings on cross-family pricing and the 300300/300500 duplicate
...
**Post-deployment cleanup (added after `fn_get_tier_price` was manually deployed to Creator dev)**: the original comment headers in `fn_get_tier_price.deluge`, `fn_calc_quote_lines.deluge`, and `fn_sync_to_sheet.deluge` caused an "Improper Statement" error when pasted into Creator's Deluge editor, traced to non-ASCII punctuation (em-dashes) in large comment blocks. All three files (plus `fn_calc_line_price_draft.deluge` and the `workflows/*.deluge` scripts, pre-emptively) were rewritten to short, single-line, ASCII-only comments with no logic change — verified via `python3 scripts/tier_price_logic_dryrun.py` (still 12/12 passing) and a brace/paren balance check. See `docs/TIER_SCHEME_CREATOR_DEPLOYMENT_RUNBOOK.md`'s deployment history note for the full incident description.
