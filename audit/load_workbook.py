#!/usr/bin/env python3
"""Load the July 2026 Acme BMS workbook into a local SQLite audit DB.
...
_WORKBOOK_NAME = "Acme BMS BMS_July 01_RSP_Distributor.xlsx"
...
def resolve_workbook_path():
    """argv > AcmeBMS_RSP_XLSX env > optional in-repo data/fixtures. No machine home paths."""
    if len(sys.argv) > 1:
        return sys.argv[1]
    env_path = os.environ.get("AcmeBMS_RSP_XLSX", "").strip()
    if env_path:
        return env_path
    candidates = [
    ...
    for path in candidates:
        if os.path.isfile(path):
            return path
    sys.stderr.write(
        "Workbook not found. Pass a path, or set AcmeBMS_RSP_XLSX to the RSP xlsx.\n"
    )
    sys.exit(2)
