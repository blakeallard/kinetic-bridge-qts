...
```
Where:
- `eur_list_price` = Lithium Balance EUR price for the SKU at the applicable volume break
...
### Distributor Price List (Lithium Balance)
...
**V1 (prototype):**
- Manual CSV import of Lithium Balance items into the `Items` form
...
### Tier Structure (observed from Lithium Balance price list)
...
Which tier does Bevco hold with Lithium Balance (Partner or Distributor)? This determines which discount row applies.
...
| Field | Type | Required | Notes |
|---|---|---|---|
| SKU | Text | Yes | Unique — Lithium Balance part number |
| Item_Name | Text | Yes | |
...
7. **Build Items form** — import Lithium Balance SKUs via CSV
...
| Lithium Balance price list (latest) | Available (local file) | Item import |
...
10. What is Bevco's distributor tier with Lithium Balance — Partner or Distributor?
...
19. Are there other BMS suppliers beyond Lithium Balance whose pricing needs to be modeled?
...
5. Import Lithium Balance price list into Items form via CSV
