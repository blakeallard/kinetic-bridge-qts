...
def main():
    ...
    check(_find(n, 'off', '300500') is not None, 'nbms_cmu12: 300500 service tool must be OFF (manual)')
    ...
    check(_find(x, 'held', '100985.1') is not None, 'n3bms_cmu18: 100985.1 harness must be HELD (Q2)')
    shunt = _find(x, 'included', '100684')
    check(shunt is not None and shunt['qty'] == 1, 'n3bms_cmu18: 100684 match_mcu_qty should be 1')
    check(_find(x, 'off', '300500') is not None, 'n3bms_cmu18: 300500 must be OFF (manual)')

    # --- cbms24 (non-CMU, series ignored) — Q1 holds ---
    c = expansions['cbms24']
    check(_find(c, 'held', '200300') is not None, 'cbms24: 200300 license must be HELD (Q1)')
    check(_find(c, 'held', '300300') is not None, 'cbms24: 300300 service tool must be HELD (Q1)')
    main24 = _find(c, 'included', '100924')
    check(main24 is not None and main24['qty'] == 1, 'cbms24: main 100924 qty 1')

    # --- cbms24x — family-correct license included, service tool off ---
    ...
    check(_find(cx, 'off', '300300') is not None, 'cbms24x: 300300 service tool OFF (manual)')

    # --- same SKU, different rule keyed by (kit, row) ---
    r_fixed = None
    for e in kits['nbms_cmu12']:
        if e['Component_SKU'] == '100684':
            r_fixed = e['Qty_Rule']
    r_match = None
    for e in kits['n3bms_cmu18']:
        if e['Component_SKU'] == '100684':
            r_match = e['Qty_Rule']
    check(r_fixed == 'fixed' and r_match == 'match_mcu_qty',
          'same-SKU: 100684 must be fixed in nbms_cmu12 and match_mcu_qty in n3bms_cmu18')
